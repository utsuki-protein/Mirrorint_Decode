...
package com.westunitis.utilityinfolinker3
...
//        private set
    ...
    init {
        ...
        stateSession = STATE_NONE
        mNewState = stateSession
    }
...
//        state = state
        Log.d(TAG, "updateUserInterfaceTitle() $mNewState -> $stateSession")
        mNewState = stateSession
    ...
    fun connect(device: BluetoothDevice, secure: Boolean) {
        ...
        // Cancel any thread attempting to make a connection
        if (stateSession == STATE_CONNECTING) {
            if (mConnectThread != null) {
                mConnectThread!!.cancel()
                mConnectThread = null
            }
    ...
    fun write(out: ByteArray) {
        ...
        synchronized(this) {
            if (stateSession != STATE_CONNECTED) return
            r = mConnectedThread
        }
    ...
    private inner class AcceptThread(secure: Boolean) : Thread() {
        ...
        override fun run() {
            ...
            while (stateSession != STATE_CONNECTED) {
                try {
    ...
    private inner class ConnectedThread(private val mmSocket: BluetoothSocket, socketType: String) : Thread() {
        ...
        init {
            ...
            mmInStream = tmpIn
            mmOutStream = tmpOut
            stateSession = STATE_CONNECTED
        }

        override fun run() {
            val buffer = Array(4){ByteArray(1024)}
            var bufIdx:Int = 0
            var bytes: Int
            ...
            while (stateSession == STATE_CONNECTED) {
                try {
                    ...
                    {
                        bytes = mmInStream!!.read(buffer[bufIdx], 0, 1024)
                        // Send the obtained bytes to the UI Activity
//                        var str = ""
//                        Log.d("AASSAA_debug", "ReadSize : " + bytes.toString())
//                        for (i in 0..bytes - 1) {
//                            str += "%02x,".format(buffer[bufIdx][i]).toUpperCase()
//                        }
//                        Log.d("AASSAA_debug", str)
////
                        mHandler.obtainMessage(MESSAGE_READ, bytes, bufIdx, buffer[bufIdx])
                            .sendToTarget()
...
//                        {
//                            mHandler.obtainMessage(MESSAGE_WRITE, 5, -1, "aaaaa")
//                                .sendToTarget()
